{{--
    ...
    DESCRIPTION:
    Main confirmation card showing the checkmark icon,
    order number, confirmation message, order meta strip
    (total, est. delivery, tracking), and action buttons.

    ==================================================================

    TODO (Backend Integration):
    - Replace static $order with data from the created Order model
    - Wire "Track Order" to route('orders.track', $order)
    - Wire "Continue Shopping" to route('products.index')
    - Trigger confirmation email from CheckoutController@store()

    ==================================================================
--}}

@php
    // TODO: replace with data from the created Order model
    $order = [
        'number' => 'NX-2024-48291',
        'grand_total' => '$2,728',
        'est_delivery' => 'June 30 - July 2 2026',
        'tracking_number' => 'NX48291726',
    ];
@endphp
...
<div class="lg:col-span-2 bg-white border border-gray-200 rounded-2xl p-10 shadow-sm h-fit text-center">
    ...
    <p class="text-sm text-gray-400 mb-1">Order #{{ $order['number'] }}</p>
    ...
    <div class="bg-gray-100 rounded-xl mt-6 mb-6 py-5 px-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-left sm:text-center">
        <div>
            ...
            <p class="text-sm font-bold text-gray-900">{{ $order['grand_total'] }}</p>
        </div>
        <div>
            <p class="text-xs text-gray-400 mb-1">Est. Delivery</p>
            <p class="text-sm font-bold text-gray-900">{{ $order['est_delivery'] }}</p>
        </div>
        <div>
            <p class="text-xs text-gray-400 mb-1">Tracking</p>
            <p class="text-sm font-bold text-cyan-500">{{ $order['tracking_number'] }}</p>
        </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-center gap-3">
        ...
        <button
            ...
            class="w-full sm:w-auto bg-white border border-gray-200 hover:bg-gray-50 transition-colors text-gray-700 text-sm font-semibold px-8 py-3 rounded-xl"
        >
            Continue Shopping
        </button>
    </div>
